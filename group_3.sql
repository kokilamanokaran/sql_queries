...
(sum(t4.arr_normal)+sum(t4.arr_afib)+sum(t4.arr_noise)+sum(t4.arr_others)) as total_session from
 ...
 (select created_at, id, name, email, gender_enum, dob from FB_User where created_at > now() - interval 180 day) AS t1
 ...
 group by t1.name DESC order by id DESC) as t3 JOIN FB_Workout as t4  where t3.id = t4.created_by group by id order by avg_wo_time desc limit 500) as tt 
